...
const prepareTransaction= async(params)=>{
    ...
    return new Promise(async (resolve, reject) => {  
        axios.post(`${BASE_URL}/v1/bc/bch/${NETWORK}/txs/create`, req,{headers:HEADERS}).then((response) => {
...
const signTransaction = async(hex,params)=>{
    ...
    return new Promise(async (resolve, reject) => {  
        axios.post(`${BASE_URL}/v1/bc/bch/${NETWORK}/txs/sign`, req,{headers:HEADERS}).then((response) => {
            resolve(response.data.payload.hex)
        }).catch((error) => {
            console.error("ERROR",error.message)
            return reject(error.message)
...
const broadcastSignedTransaction = async(hex)=>{
    ...
    return new Promise(async (resolve, reject) => {  
        axios.post(`${BASE_URL}/v1/bc/bch/${NETWORK}/txs/send`, req,{headers:HEADERS}).then((response) => {
